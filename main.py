...
TABELA_CASA_FORA23 = api_banco.pega_dados(r"dataset/casa_fora23.xlsx")
...
DADOS_CASA_FORA = api_banco.pega_tabela_casa_fora()

TEMPORADA_2023 = api_banco.pega_dados(r"dataset/tabela23.xlsx")
TEMPORADA_2022, CASA_FORA22 = api_banco.divide_temporada(DADOS, DADOS_CASA_FORA, 2022)
TEMPORADA_2021, CASA_FORA21 = api_banco.divide_temporada(DADOS, DADOS_CASA_FORA, 2021)
TEMPORADA_2020, CASA_FORA20 = api_banco.divide_temporada(DADOS, DADOS_CASA_FORA, 2020)
TEMPORADA_2019, CASA_FORA19 = api_banco.divide_temporada(DADOS, DADOS_CASA_FORA, 2019)
TEMPORADA_2018, CASA_FORA18 = api_banco.divide_temporada(DADOS, DADOS_CASA_FORA, 2018)
TEMPORADA_2017, CASA_FORA17 = api_banco.divide_temporada(DADOS, DADOS_CASA_FORA, 2017)
TEMPORADA_2016, CASA_FORA16 = api_banco.divide_temporada(DADOS, DADOS_CASA_FORA, 2016)
TEMPORADA_2015, CASA_FORA15 = api_banco.divide_temporada(DADOS, DADOS_CASA_FORA, 2015)
TEMPORADA_2014, CASA_FORA14 = api_banco.divide_temporada(DADOS, DADOS_CASA_FORA, 2014)
...
def main():
    ...
    #, "search"
        ...
        #c = conexao.cursor()
        #c.execute('DROP TABLE tb_jogos_mais_cartoes')
        #conexao.commit()
        #MAIS_CARTOES.to_sql('tb_jogos_mais_cartoes', conexao, if_exists='replace')
        #query = "SELECT * FROM tb_jogos_mais_cartoes"
        #df = pd.read_sql_query(query, conexao)
        #conexao.close()
        #st.table(df)
    if selected == "Inicio":
        view.mostrar_tabela_classificacao(TEMPORADA_2023, TABELA_CASA_FORA23, 2023)
        j1, j2 = st.columns(2)
        ...
        proba = view.mostra_probabilidades(EQUIPES, time1, time2)
    ...
    if selected == "Temporadas":
        ...
        if opcao == "Brasileiro 2022":
            view.mostrar_tabela_classificacao(TEMPORADA_2022, CASA_FORA22, 2022)
            st.markdown('---')
            st.title("Resumo Estatístico")
            view.mostra_jogos(TEMPORADA_2022, CASA_FORA22)
            st.markdown('---')
            view.mostra_cartoes(TEMPORADA_2022)
            st.markdown('---')
            view.mostra_dados_ataques(TEMPORADA_2022, CASA_FORA22, ESTATISTICAS, 2022)
            st.markdown('---')
            ...
            view.mostrar_tabela_classificacao(TEMPORADA_2021, CASA_FORA21, 2021)
            st.markdown('---')
            st.title("Resumo Estatístico")
            view.mostra_jogos(TEMPORADA_2021, CASA_FORA21)
            st.markdown('---')
            view.mostra_cartoes(TEMPORADA_2021)
            st.markdown('---')
            view.mostra_dados_ataques(TEMPORADA_2021, CASA_FORA21, ESTATISTICAS, 2021)
            st.markdown('---')
            view.mostra_dados_defesas(TEMPORADA_2021)
            st.markdown('---')
            #info.mostra_estatisticas_diversas(ESTATISTICAS, 2021)
        elif opcao == "Brasileiro 2020":
            view.mostrar_tabela_classificacao(TEMPORADA_2020, CASA_FORA20, 2020)
            st.markdown('---')
            st.title("Resumo Estatístico")
            view.mostra_jogos(TEMPORADA_2020, CASA_FORA21)
            st.markdown('---')
            view.mostra_cartoes(TEMPORADA_2020)
            st.markdown('---')
            view.mostra_dados_ataques(TEMPORADA_2020, CASA_FORA21, ESTATISTICAS, 2020)
            st.markdown('---')
            view.mostra_dados_defesas(TEMPORADA_2020)
            st.markdown('---')
            #info.mostra_estatisticas_diversas(ESTATISTICAS, 2020)
        elif opcao == "Brasileiro 2019":
            view.mostrar_tabela_classificacao(TEMPORADA_2019, CASA_FORA19, 2019)
            st.markdown('---')
            st.title("Resumo Estatístico")
            view.mostra_jogos(TEMPORADA_2019, CASA_FORA21)
            st.markdown('---')
            view.mostra_cartoes(TEMPORADA_2019)
            st.markdown('---')
            view.mostra_dados_ataques(TEMPORADA_2019, CASA_FORA21, ESTATISTICAS, 2019)
            st.markdown('---')
            view.mostra_dados_defesas(TEMPORADA_2019)
            st.markdown('---')
            #info.mostra_estatisticas_diversas(ESTATISTICAS, 2019)
        elif opcao == "Brasileiro 2018":
            view.mostrar_tabela_classificacao(TEMPORADA_2018, CASA_FORA18, 2018)
            st.markdown('---')
            st.title("Resumo Estatístico")
            view.mostra_jogos(TEMPORADA_2018, CASA_FORA21)
            st.markdown('---')
            view.mostra_cartoes(TEMPORADA_2018)
            st.markdown('---')
            view.mostra_dados_ataques(TEMPORADA_2018, CASA_FORA21, ESTATISTICAS, 2018)
            st.markdown('---')
            view.mostra_dados_defesas(TEMPORADA_2018)
            st.markdown('---')
            #info.mostra_estatisticas_diversas(ESTATISTICAS, 2018)
        elif opcao == "Brasileiro 2017":
            view.mostrar_tabela_classificacao(TEMPORADA_2017, CASA_FORA17, 2017)
            st.markdown('---')
            st.title("Resumo Estatístico")
            view.mostra_jogos(TEMPORADA_2017, CASA_FORA21)
            st.markdown('---')
            view.mostra_cartoes(TEMPORADA_2017)
            st.markdown('---')
            view.mostra_dados_ataques(TEMPORADA_2017, CASA_FORA21, ESTATISTICAS, 2017)
            st.markdown('---')
            view.mostra_dados_defesas(TEMPORADA_2017)
            st.markdown('---')
            #info.mostra_estatisticas_diversas(ESTATISTICAS, 2017)
        elif opcao == "Brasileiro 2016":
            view.mostrar_tabela_classificacao(TEMPORADA_2016, CASA_FORA16, 2016)
            st.markdown('---')
            st.title("Resumo Estatístico")
            view.mostra_jogos(TEMPORADA_2016, CASA_FORA21)
            st.markdown('---')
            view.mostra_cartoes(TEMPORADA_2016)
            st.markdown('---')
            view.mostra_dados_ataques(TEMPORADA_2016, CASA_FORA21, ESTATISTICAS, 2016)
            st.markdown('---')
            view.mostra_dados_defesas(TEMPORADA_2016)
            st.markdown('---')
            #info.mostra_estatisticas_diversas(ESTATISTICAS, 2016)
        elif opcao == "Brasileiro 2015":
            view.mostrar_tabela_classificacao(TEMPORADA_2015, CASA_FORA15, 2015)
            st.markdown('---')
            st.title("Resumo Estatístico")
            view.mostra_jogos(TEMPORADA_2015, CASA_FORA21)
            st.markdown('---')
            view.mostra_cartoes(TEMPORADA_2015)
            st.markdown('---')
            view.mostra_dados_ataques(TEMPORADA_2015, CASA_FORA21, ESTATISTICAS, 2015)
            st.markdown('---')
            view.mostra_dados_defesas(TEMPORADA_2015)
            st.markdown('---')
            #info.mostra_estatisticas_diversas(ESTATISTICAS, 2022)
        elif opcao == "Brasileiro 2014":
            view.mostrar_tabela_classificacao(TEMPORADA_2014, CASA_FORA14, 2014)
            st.markdown('---')
            st.title("Resumo Estatístico")
            view.mostra_jogos(TEMPORADA_2014, CASA_FORA21)
            st.markdown('---')
            view.mostra_cartoes(TEMPORADA_2014)
            st.markdown('---')
            view.mostra_dados_ataques(TEMPORADA_2014, CASA_FORA21, ESTATISTICAS, 2014)
            st.markdown('---')
